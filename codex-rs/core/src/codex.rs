...
pub(crate) struct TurnContext {
    ...
    /// Baseline user instructions captured at session start (pre‑modes).
    pub(crate) base_user_instructions: Option<String>,
...
impl Session {
    async fn new(
        ...
        let turn_context = TurnContext {
            ...
            base_instructions,
            base_user_instructions: user_instructions,
            approval_policy,
            sandbox_policy,
            ...
            Op::OverrideTurnContext {
                ...
                let new_turn_context = TurnContext {
                    ...
                    user_instructions: user_instructions
                        .clone()
                        .or_else(|| prev.user_instructions.clone()),
                    base_instructions: prev.base_instructions.clone(),
                    base_user_instructions: prev.base_user_instructions.clone(),
                ...
                // If user_instructions were overridden, rebuild the initial
                // `<user_instructions>` message in the in‑memory history.
                if user_instructions.is_some() {
                    ...
                    // Build fresh initial context from the updated turn context
                    let initial_items = sess.build_initial_context(turn_context.as_ref());

                    // Retain all items except the session prefix messages
                    // (<user_instructions>, <environment_context>), which are
                    // regenerated by build_initial_context.
                    let mut retained: Vec<ResponseItem> = Vec::new();
                    for item in snapshot.into_iter() {
                        match &item {
                            ResponseItem::Message { role, content, .. } if role == "user" => {
                                if let Some(text) =
                                    crate::codex::compact::content_items_to_text(content)
                                    && crate::codex::compact::is_session_prefix_message(&text)
                                {
                                    // skip prefix
                                    continue;
                                }
                    ...
                    // Replace history: new initial context + retained items
                    let mut new_history = initial_items;
            ...
            Op::UserTurn {
                ...
                if let Err(items) = sess.inject_input(items).await {
                    ...
                    let fresh_turn_context = TurnContext {
                        ...
                        base_user_instructions: turn_context.base_user_instructions.clone(),
                        approval_policy,
                        sandbox_policy,
...
mod tests {
    ...
    pub(crate) fn make_session_and_context() -> (Session, TurnContext) {
        ...
        let turn_context = TurnContext {
            ...
            base_user_instructions: config.user_instructions.clone(),
            approval_policy: config.approval_policy,
            sandbox_policy: config.sandbox_policy.clone(),
            shell_environment_policy: config.shell_environment_policy.clone(),
            tools_config,
