...
本节给出不依赖任何脚本的“纯 Git”最佳实践。若与后文脚本化说明存在差异，请以本节为准。

### 分支角色
- `upstream/main`：上游开发分支（版本号通常为 `0.0.0`）。
- `main`（本仓库）：镜像 `upstream/main`，仅作基线同步；不合入“发布用版本号提交”。
- `dev`：你的长期开发分支，承载 fork 特性；定期基于 `upstream/main` 变基。
- `release/fork-X.Y.Z`：一次性发布分支，严格从上游标签 `rust-vX.Y.Z` 切出并叠加 fork 特性；只在这里修改版本号。
- `stable`（可选）：指向最近一次对外发布的 fork 标签，便于外部/CI 固定引用。
...
### 基于上游标签发布自己的版本（不带入上游后续提交）
目标：以 `rust-vX.Y.Z` 为基线，仅叠加 fork 私有改动，然后在发布分支上修改版本号为 `X.Y.Z-fork.N` 并打标签。

1) 从上游标签切发布分支
```bash
git fetch upstream --tags
git checkout -b release/fork-X.Y.Z rust-vX.Y.Z
```

2) 批量迁移 fork 私有提交到发布分支（推荐 rebase --onto）
```bash
# 一把梭：把 [upstream/main..dev] 的提交重放到 rust-vX.Y.Z 上
git rebase --onto rust-vX.Y.Z upstream/main release/fork-X.Y.Z
...
可替代方案：
- format-patch + am（导出补丁集再应用）：
  ```bash
  git format-patch --no-stat --topo-order upstream/main..dev -o ../patches-fork-X.Y.Z
  git am -3 ../patches-fork-X.Y.Z/*.patch
  ```
- 脚本化 cherry-pick（自动枚举私有提交顺序应用）：
  ```bash
  COMMITS=$(git rev-list --reverse --no-merges --cherry-pick --right-only upstream/main...dev)
  for c in $COMMITS; do
    git cherry-pick -x "$c" || { echo '解决冲突后执行: git cherry-pick --continue'; exit 1; }
  done
  ```

3) 仅在发布分支上修改版本号并打标签
```bash
# 编辑 codex-rs/Cargo.toml，将 [workspace.package].version 改为 X.Y.Z-fork.N
# 注：开发分支维持 0.0.0，发布分支才 bump 版本

git add codex-rs/Cargo.toml codex-rs/Cargo.lock  # 若仅版本号变化产生锁文件漂移，可选择不提交锁文件
git commit -m "Release fork X.Y.Z-fork.N"

# 打发布标签（与版本号严格一致，便于自动化）
git tag -a rust-vX.Y.Z-fork.N -m "Release fork X.Y.Z-fork.N"
git push origin release/fork-X.Y.Z
git push origin rust-vX.Y.Z-fork.N
```

4) 不把“发布用版本号提交”合回开发线
- 与上游做法一致：`release/fork-X.Y.Z` 为一次性分支；版本号提交和可能的锁文件变化留在发布线，不回合到 `dev` 或 `main`。

### 版本号与锁文件原则
- 开发线（`main`/`dev`）：保持 `0.0.0`，避免无谓的 `Cargo.lock` 漂移。
- 发布线（`release/fork-X.Y.Z`）：将版本改为 `X.Y.Z-fork.N`；如未更改第三方依赖，仅版本号变化引起的锁文件差异可不提交。
- 只有在新增/变更依赖时，才应提交更新后的 `Cargo.lock`。
