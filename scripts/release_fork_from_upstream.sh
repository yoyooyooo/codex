...
RESUME="false"
STRATEGY="cherry-pick"   # cherry-pick | rebase-onto
REPLAY_BASE="auto"        # auto | upstream | release
...
Options
  ...
  --strategy <name>       Replay strategy: cherry-pick (default) or rebase-onto
  --replay-base <name>    Base for diff/replay: auto (default), upstream, or release
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --resume)
      RESUME="true" ;;
    --strategy)
      shift; STRATEGY="${1:-}" || true ;;
    --replay-base)
      shift; REPLAY_BASE="${1:-}" || true ;;
    --dry-run)
      DRY_RUN="true" ;;
    -h|--help)
      usage; exit 0 ;;
...
update_version_and_lock() {
  ...
  if [[ "$DRY_RUN" == "true" ]]; then
    ...
    git add codex-rs/Cargo.toml codex-rs/Cargo.lock 2>/dev/null || true
    # Fallback adds in case some files are absent (e.g., no lock update)
    git add codex-rs/Cargo.toml 2>/dev/null || true
    git commit -m "chore: bump version to ${full_ver} for fork release"
  fi
}

apply_fork_patches() {
  local base_ref="$1"   # e.g. upstream/main or release/fork-X.Y.Z
  ...
  if [[ "$DRY_RUN" == "true" ]]; then
    echo "[dry-run] git fetch ${UPSTREAM_REMOTE} ${UPSTREAM_BASE} --no-tags (ensure upstream base exists)"
  else
  ...
  if [[ "$base_ref" == "$BRANCH" ]]; then
    # Incremental replay: intersect two sets (preserve order from upstream diff)
    # A = unique(dev vs upstream)
    local list_a
    list_a=$(git rev-list --reverse --right-only --cherry-pick "${UPSTREAM_REMOTE}/${UPSTREAM_BASE}...${head_ref}" 2>/dev/null || true)
    # B = unique(dev vs release)
    local list_b
    list_b=$(git rev-list --reverse --right-only --cherry-pick "${BRANCH}...${head_ref}" 2>/dev/null || true)
    # Intersection: keep items from list_a that also appear in list_b (by commit id)
    if [[ -n "$list_a" && -n "$list_b" ]]; then
      commits=$(printf "%s\n" "$list_a" | grep -F -x -f <(printf "%s\n" "$list_b") || true)
    else
      commits=""
    fi
  else
...
# Handle in-progress replay (cherry-pick or rebase) before any branch switching
if replay_in_progress; then
  echo "检测到正在进行的 cherry-pick 操作。"
  if [[ "$RESUME" == "true" || "$RESUME_CHERRY" == "true" ]]; then
    echo "尝试继续进行中的操作（优先 rebase，其次 cherry-pick）"
    if [[ "$DRY_RUN" == "true" ]]; then
      echo "[dry-run] git rebase --continue || git cherry-pick --continue"
      if [[ "$RESUME" == "true" ]]; then
    ...
    if [[ "$DRY_RUN" == "true" ]]; then
      echo "[dry-run] git rebase --abort || git rebase --quit || git cherry-pick --abort || git cherry-pick --quit"
    else
      git rebase --abort 2>/dev/null || git rebase --quit 2>/dev/null || git cherry-pick --abort 2>/dev/null || git cherry-pick --quit 2>/dev/null || true
    fi
  else
...
# Decide effective replay base and compute range base ref
# Determine if release branch already exists
BRANCH_EXISTS="false"
if git rev-parse -q --verify "refs/heads/${BRANCH}" >/dev/null; then
  BRANCH_EXISTS="true"
fi

EFFECTIVE_REPLAY_BASE="$REPLAY_BASE"
if [[ "$REPLAY_BASE" == "auto" ]]; then
  if [[ "$BRANCH_EXISTS" == "true" ]]; then
    EFFECTIVE_REPLAY_BASE="release"
  else
    EFFECTIVE_REPLAY_BASE="upstream"
  fi
fi

RANGE_BASE_REF="${UPSTREAM_REMOTE}/${UPSTREAM_BASE}"
if [[ "$EFFECTIVE_REPLAY_BASE" == "release" ]]; then
  RANGE_BASE_REF="${BRANCH}"
fi

# If user forced release base but branch doesn't exist, abort
if [[ "$REPLAY_BASE" == "release" && "$BRANCH_EXISTS" != "true" ]]; then
  echo "ERROR: --replay-base=release 需要已存在的分支 ${BRANCH}。请先进行首发，或使用 --replay-base upstream/auto。" >&2
  exit 1
fi

# If using release base but strategy is rebase-onto, fall back to cherry-pick
STRATEGY_EFFECTIVE="$STRATEGY"
if [[ "$EFFECTIVE_REPLAY_BASE" == "release" && "$STRATEGY" == "rebase-onto" ]]; then
  echo "提示：在 release 基线上不支持 rebase-onto，改用 cherry-pick 增量重放。"
  STRATEGY_EFFECTIVE="cherry-pick"
fi
...
if [[ "$EFFECTIVE_REPLAY_BASE" == "release" ]]; then
  echo "  Release branch: ${BRANCH} (from release head)"
elif [[ "$STRATEGY" == "rebase-onto" ]]; then
  echo "  Release branch: ${BRANCH} (from ${MAIN_BRANCH})"
else
...
if [[ "$RESUME" == "true" ]]; then
  ...
  echo "  Strategy      : ${STRATEGY_EFFECTIVE}"
  echo "  Replay base   : ${EFFECTIVE_REPLAY_BASE}"
  echo "  Apply patches : ${APPLY_PATCHES} (range: ${RANGE_BASE_REF}..${MAIN_BRANCH})"
fi
  ...
  if [[ "$DRY_RUN" == "true" ]]; then
    if [[ "$EFFECTIVE_REPLAY_BASE" == "release" ]]; then
      if [[ "$BRANCH_EXISTS" == "true" ]]; then
        ...
        echo "[dry-run] (release-base) 不重置到上游 tag"
      else
        echo "[dry-run] (release-base) 需要已有分支 ${BRANCH}，否则无法增量重放"
      fi
    elif [[ "$STRATEGY" == "rebase-onto" ]]; then
      if [[ "$BRANCH_EXISTS" == "true" ]]; then
        echo "[dry-run] git switch ${BRANCH}"
        if [[ "$NO_RESET" == "true" ]]; then
          ...
          echo "[dry-run] git reset --hard ${MAIN_BRANCH}   # reuse existing branch, reset to local ${MAIN_BRANCH}"
        fi
      else
        echo "[dry-run] git switch -c ${BRANCH} ${MAIN_BRANCH}"
      fi
    else
      if [[ "$BRANCH_EXISTS" == "true" ]]; then
        echo "[dry-run] git switch ${BRANCH}"
    ...
    if [[ "$EFFECTIVE_REPLAY_BASE" == "release" ]]; then
      if [[ "$BRANCH_EXISTS" == "true" ]]; then
        git switch "${BRANCH}"
        # release-base: never reset to avoid losing prior releases
      else
        echo "ERROR: release-base 需要已有分支 ${BRANCH}。" >&2
        exit 1
      fi
    elif [[ "$STRATEGY" == "rebase-onto" ]]; then
      if [[ "$BRANCH_EXISTS" == "true" ]]; then
...
if [[ "$RESUME" != "true" && "$APPLY_PATCHES" == "true" ]]; then
  if [[ "$STRATEGY_EFFECTIVE" == "rebase-onto" ]]; then
    echo "将以 rebase --onto 方式重放：${RANGE_BASE_REF}..${MAIN_BRANCH} → 基线 ${UPSTREAM_TAG}"
    # Preview commit list
    commits_preview=$(git rev-list --reverse --no-merges "${RANGE_BASE_REF}..${MAIN_BRANCH}" 2>/dev/null || true)
    if [[ -n "$commits_preview" ]]; then
      echo "Commit list to rebase (oldest first):"
      while IFS= read -r c; do
        [[ -z "$c" ]] && continue
        echo "  $(git show -s --format='%h %s' "$c")"
      done <<< "$commits_preview"
    else
      echo "No commits to rebase (range ${RANGE_BASE_REF}..${MAIN_BRANCH} is empty)"
    fi
    if [[ "$DRY_RUN" == "true" ]]; then
      echo "[dry-run] git rebase --onto ${UPSTREAM_TAG} ${RANGE_BASE_REF} ${BRANCH}"
    else
      if ! git rebase --onto "${UPSTREAM_TAG}" "${RANGE_BASE_REF}"; then
        echo "❌ rebase --onto 过程中发生冲突，请解决后运行 'git rebase --continue' 或使用 '--resume' 收尾。" >&2
        exit 2
      fi
    fi
  else
    apply_fork_patches "${RANGE_BASE_REF}" "${MAIN_BRANCH}"
  fi
else
