...
if cherry_pick_in_progress; then
  ...
  if [[ "$RESUME_CHERRY" == "true" ]]; then
    echo "尝试执行: git cherry-pick --continue"
    if [[ "$DRY_RUN" == "true" ]]; then
      echo "[dry-run] git cherry-pick --continue"
      echo "[dry-run] 跳过提交重放与分支切换，仅进行后续版本与打标签步骤"
    else
      if ! git cherry-pick --continue; then
        echo "❌ 无法继续 cherry-pick。请解决冲突后再次运行 --resume-cherry 或手动继续。" >&2
        exit 2
      fi
    fi
...
if [[ "$APPLY_PATCHES" == "true" && "$USER_SET_MAIN" == "false" ]]; then
